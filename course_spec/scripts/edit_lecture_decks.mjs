...
const episodes = [
  ...
  },
  {
    ...
    slides: [
      ...
      ["Grow the process, keep the guarantees", [["Agent-assisted review", "Trace table and package dependencies\nUpdate tests with the requirement\nReview generated diff"], ["What comes next", "Debug edge cases in production\nHandle employee cancellation\nRecover stuck workflow balances"]]],
    ],
  },
  {
    dir: "13_architecture_refactoring_roles",
    title: "AI-Assisted Debugging & Workflow Recovery",
    subtitle: "Handling cancellation, workflow termination, and balance recovery",
    slides: [
      ["Edge cases in production", [["The happy path trap", "Submissions work in isolation\nReal systems encounter cancellations\nWorkflows terminate unexpectedly"], ["System integrity", "Compensating transactions\nSynchronized workflow states\nGuaranteed balance invariants"]]],
      ["Employee cancellation", [["Security gate", "CAN_CANCEL_REQUEST checks ownership\nStart date must be in future\nPending statuses only"], ["Compensating actions", "Release PENDING_DAYS to available\nCancel APEX workflow instance\nLog CANCELLED audit event"]]],
      ["The double-deduction trap", [["Root cause", "CREATE_REQUEST reserves pending days\nGET_AVAILABLE_DAYS returns remaining\nWorkflow check required 2x days"], ["The fix", "Add REQUESTED_DAYS back in switch\nCheck available >= 0\nEliminate premature termination"]]],
      ["Automated balance recovery", [["Orphaned workflows", "Detect terminated or faulted runs\nIdentify locked pending days\nReconcile employee balances"], ["What comes next", "Run full database assertions\nExercise both applications\nPrepare guarded handoff"]]],
    ],
  },
  {
