...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "DEMO"."HR_AI_PKG" AS
    ...
    BEGIN
        ...
        IF l_user_id IS NULL THEN
            RETURN TO_CLOB(JSON_OBJECT('error' VALUE 'No authenticated session or user profile not found'));
        END IF;
    ...
    BEGIN
        l_user_id := hr_user_pkg.current_user_id;
        IF l_user_id IS NULL THEN
            RETURN TO_CLOB(JSON_OBJECT('error' VALUE 'No authenticated session'));
        END IF;
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RETURN TO_CLOB(JSON_OBJECT('error' VALUE 'No balance record found for leave type ' || p_leave_type_code || ' in year ' || l_year));
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RETURN TO_CLOB(JSON_OBJECT('error' VALUE 'Leave request #' || p_request_id || ' not found or unauthorized'));
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RETURN TO_CLOB(JSON_OBJECT('status' VALUE 'ERROR', 'message' VALUE SQLERRM));
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK TO sp_ai_create_leave;
            RETURN TO_CLOB(JSON_OBJECT('status' VALUE 'ERROR', 'message' VALUE SQLERRM));
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK TO sp_ai_cancel_leave;
            RETURN TO_CLOB(JSON_OBJECT('status' VALUE 'ERROR', 'message' VALUE SQLERRM));
    ...
    FUNCTION generate_request_summary(
        ...
        l_caller_id      NUMBER;
    BEGIN
        l_caller_id := hr_user_pkg.current_user_id;
        IF l_caller_id IS NULL AND NOT hr_auth_pkg.is_admin THEN
            RETURN TO_CLOB('Unauthorized: No authenticated session.');
        END IF;
        ...
        SELECT u.full_name, t.leave_type_name, r.requested_days,
          ...
          JOIN hr_leave_types t ON t.leave_type_id = r.leave_type_id
         WHERE r.request_id = p_request_id
           AND (r.user_id = l_caller_id OR hr_auth_pkg.is_admin);
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RETURN TO_CLOB('Request #' || p_request_id || ' not found or unauthorized.');
