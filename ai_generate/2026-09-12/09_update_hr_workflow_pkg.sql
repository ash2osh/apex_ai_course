...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "DEMO"."HR_WORKFLOW_PKG" AS
    ...
    BEGIN
        ...
        IF l_active_cnt > 0 THEN
            ...
            BEGIN
                ...
                UPDATE hr_leave_requests
                   SET workflow_id = l_workflow_id,
                       ...
                       updated_by  = COALESCE(apex_application.g_user, l_username, 'SYSTEM')
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    workflow_fault(p_request_id, 'Workflow start exception: ' || SQLERRM);
                    RAISE_APPLICATION_ERROR(-20036, 'Failed to start leave approval workflow: ' || SQLERRM);
            END;
        ELSE
            -- Reject request initiation if no active workflow version exists to prevent stranding balance reservations
            workflow_fault(p_request_id, 'Workflow LEAVE_APPROVAL has no active version in application 200');
            RAISE_APPLICATION_ERROR(-20035, 'Workflow LEAVE_APPROVAL has no active version in application 200.');
        END IF;
    ...
    BEGIN
        ...
        IF NOT hr_auth_pkg.can_approve_request(p_actor_username, p_request_id) THEN
            RAISE_APPLICATION_ERROR(-20031, 'User ' || NVL(p_actor_username, 'ANONYMOUS') || ' is not authorized to act on request #' || p_request_id);
        END IF;

        -- 2. Lock request row for update and check current status
        ...
        -- 3. Idempotency guard: do not re-transition terminal records
        IF l_status IN ('APPROVED', 'REJECTED', 'CANCELLED') THEN
            RAISE_APPLICATION_ERROR(-20032, 'Leave request #' || p_request_id || ' is already in terminal status: ' || l_status);
        END IF;
